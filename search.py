...
class Node:
    ...
    def __init__(self, state, parent=None, action=None, path_cost=0, estimation=None):
        """Create a search tree Node, derived from a parent by an action."""
        update(self, state=state, parent=parent, action=action,
               path_cost=path_cost, depth=0,estimation=estimation)
        if parent:
    ...
    def expand(self, problem):
        ...
        return [Node(next, self, act, problem.path_cost(self.path_cost, self.state, act, next), problem.path_cost(self.path_cost, self.state, act, next) + problem.h(Node(next)))
...
def no_informada(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return tree_search(problem, NoInformada())


def informada(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return tree_search(problem, Informada())
